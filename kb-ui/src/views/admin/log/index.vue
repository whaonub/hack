<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item filter-button" type="primary" v-waves icon="search" @click="handleFilter">搜索</el-button>
    </div>
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row style="width: 99%">
      ...
      <el-table-column label="类型" align="center">
        <template slot-scope="scope">
          <span>
            <el-tag type="success"  v-if="scope.row.type == 0">{{ scope.row.type | typeFilter }}</el-tag>
            <el-tag type="danger"  v-if="scope.row.type ==9">{{ scope.row.type | typeFilter }}</el-tag>
      ...
      <el-table-column label="请求接口" show-overflow-tooltip align="center">
        <template slot-scope="scope">
