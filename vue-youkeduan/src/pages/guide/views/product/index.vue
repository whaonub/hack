        ...
        <my-body>
            <ul class="product">
                <li class="product__li" v-for="(item, key) in list" :key="key">
                    ...
                    <div class="product__btn">删除</div>
                </li>
            </ul>
        </my-body>
        <div class="foot">
            <div class="foot__left" @click="routerJump('productDetail')">添加我的产品</div>
            <div class="foot__right" @click="routerJump('productShare')">添加共享产品</div>
        </div>
    </my-page>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                const self = this
                self.api.getProductList().then(res => {
                    self.list = res.data
                })
            },
        },
